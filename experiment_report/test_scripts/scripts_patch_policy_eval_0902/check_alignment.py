...
for i in (5000, 12345, 20000, 33333):
    ...
    # With n_obs_steps == 1 (act_eef) resolve_delta_timestamps returns no observation
    # window at all, so the state comes back flat -- give it the missing time axis.
    if n_obs == 1 and h["observation.state"].ndim == 1:
        h = dict(h, **{"observation.state": h["observation.state"][None]})
    ...
    if n_obs > 1 and not torch.equal(h["observation.state"][0], f["observation.state"]):
        neg_ok += 1
    # delta 0 of the action window is this frame's own action
    assert torch.equal(h["action"][a_off], f["action"]), i
    # Scoring from index 0 instead would compare against an action n_obs-1 ticks in the
    # past.  That is only *detectable* at an anchor where the command actually moved: the
    # EEF recordings hold the target pose for long stretches, so a duplicate here is a
    # property of the data, not a mis-indexed window.  Requiring it at every anchor (as
    # the joint-space version did) is what made this check fail on batch_success_533_eef.
    if n_obs > 1 and not torch.equal(h["action"][0], f["action"]):
        neg_ok += 1
    # the chunk that gets scored must be the future, not the past
    chunk = getattr(cfg, "action_chunk_size", None) or cfg.chunk_size
    assert h["action"].shape[0] - a_off == chunk, h["action"].shape
if n_obs > 1:
    assert neg_ok > 0, "every probed anchor is a held command; the negative control is blind here"
    print(f"negative control fired at {neg_ok} of 8 probes (a held command cannot fire it)")
