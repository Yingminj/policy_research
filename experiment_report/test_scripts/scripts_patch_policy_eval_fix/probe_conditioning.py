...
Fork of `scripts_patch_policy_probe/probe_patch_conditioning.py`, extended with the two
`observation.state` interventions the earlier checkpoints had no use for: they all ran
`use_robot_state=False`, and one of the checkpoints measured here does not.
...
Three questions:
  ...
  2. **Can the policy locate the arm?** Without proprioception the chunk's first action has to
     be inferred from pixels. `first_action_vs_state` compares it to the measured pose the
     action should start from; the demonstrations' own first action is the ceiling.
  3. **When `use_robot_state=True`, is the state token actually read?** `state_swapped` hands the
     policy another anchor's proprioception with the images unchanged, and `zero_state` hands
     it the normaliser's mid-range. A delta near zero on both means the token is decorative.
...
def main() -> None:
    ...
    interventions = [
        ...
        *(["state_swapped", "zero_state", "state_history_reversed"] if cfg.use_robot_state else []),
    ...
    for bi, batch in enumerate(loader):
        ...
        for name in interventions[1:]:
            ...
            if name == "all_cams_swapped":
                ...
                for c in cams:
                    v[c] = proc[c].flip(1)
            elif name == "state_swapped":
                v[OBS_STATE_KEY] = proc[OBS_STATE_KEY].roll(1, dims=0)
            elif name == "zero_state":
                v[OBS_STATE_KEY] = torch.zeros_like(proc[OBS_STATE_KEY])
            elif name == "state_history_reversed":
                v[OBS_STATE_KEY] = proc[OBS_STATE_KEY].flip(1)
