...
> **2026-08-13（晚）更新**：按 §10 第 4 条试了 `n_action_steps: 8→4` 和
> `chunk_interval_s: 0.25→0.2667`，两者都无效，见 §11。
> **§11 作废了 §10 第 4 条的第一小项**：`chunk_interval_s` 在本部署下是死代码
> （节奏由服务端 `need_new_chunk` 驱动）；`n_action_steps` 减半让接缝更小但更密，
> 尺度无关指标持平。接缝只剩 RTC 或结构性修改两条路。
>
...
## 11. 实验四：`n_action_steps=4` + `chunk_interval_s=0.2667` 复测（2026-08-13 晚）

按 §10 第 4 条做了两处改动，真机反馈"没有区别"。测量证实了这个主观判断，
并且**推翻了 §10 第 4 条的第一小项**：缩短重规划周期治不了接缝。

数据：`record_chunk.txt`，137 chunk × 4 步 = 548 帧，137/137 带
`Source: raw policy output (pre-filter)`，同一个 200k checkpoint。
归档在 `scripts_vita_chunk_0813_n4/`。

### 11.1 `chunk_interval_s` 这一改**在结构上就是空操作**

`inference/chunk.py:169-206`：时间节流那一段只有在观测里**没有** `need_new_chunk`
时才会走到。而 `robots/marvain_m6_http/marvain_m6_http.py:598-600` 只要服务端下发
该字段就会原样透传，`strategies/core.py:363-371` 再把它放进 `obs_frame`。
也就是说本部署下 chunk 节奏**完全由机器人服务端的 `need_new_chunk` 事件驱动**，
`chunk_interval_s` 是死代码，0.25 改成 0.2667 不可能有任何效果。

想自己确认：跑的时候看日志里有没有出现过 `Using chunk_interval_s`（`chunk.py:206`）。
没有 = 一次都没走过时间节流分支。

§5.4c 里"8 帧 @30fps = 0.2667s，配置写 0.25 有 6% 失配"这个观察本身没错，
但那个失配落在**服务端**的执行节拍上，改客户端的 `chunk_interval_s` 够不着它。
要改得去服务端。

### 11.2 `n_action_steps=4` 确实生效了，但抖动是尺度无关的

绝对量全线下降，但机器人这一段本身走得就慢——每帧手臂 state 行程
0.0470 rad（n=8）→ **0.0182 rad**（n=4），慢了 2.6×。
把尺度除掉之后，**比值全部持平或略微变差**：

| 手臂，尺度无关指标 | n=8 | n=4（新） |
|---|---|---|
| 接缝占总指令路径的比例 | 40.2% | **42.0%** |
| chunk 内 path/net（n=8 取前 4 步对齐窗长） | 1.543 | **1.741** |
| 非单调 chunk 占比 | 29.0% | **32.5%** |
| 连续流反转率 | 37.5% | **46.3%** |
| 连续流 lag-1 自相关 | +0.030 | **−0.090** |

接缝/帧内比确实从 4.70× 降到 2.17×，但**接缝出现的频率翻倍**，
两者恰好抵消：接缝占总路径的份额没动，甚至略涨。这就是"没有区别"的来源。

### 11.3 为什么必然如此

§9 第 3 条已经测出接缝**不是跟踪滞后**（与滞后相关仅 +0.057），
而是每次推理的**首帧偏移**——`|action0 − state|` 是训练集的 1.86×。
首帧偏移是"一次推理"的性质，跟这次推理的结果要执行多久无关。
把执行窗口砍半，不会让单次偏移变小，只会让你**更频繁地付这笔钱**。

实测正好对上：接缝绝对量降了 4.8×（0.0352 → 0.0073），
而每帧运动量只降了 2.6×——单次接缝相对运动确实小了约 1.9×，
但频率 ×2 把它吃回去了。

### 11.4 一个附带的坏信号（有混淆，仅作提示）

n=4 下每个 chunk 的运动预算被首帧偏移吃光了：

| | 每 chunk 指令净位移 | 每 chunk `\|action0 − state\|` | 比值 |
|---|---|---|---|
| n=8 | 0.6734 | 0.5291 | 0.79 |
| n=4 | 0.1151 | 0.1158 | **1.01** |

比值到 1.01 意味着：一个 chunk 走完的距离，正好等于它开头就欠着的那段偏移——
整段 chunk 都在补上一次重规划留下的错位，没有净进展。这与观察到的"变慢"一致。

**但这条有混淆**：n=4 这次录制里右夹爪全程没开
（state `[0.0151, 0.0185]`，action `[−0.0045, 0.0285]`），
而 n=8 那次右夹爪到过 1.30 —— 两次录的不是任务的同一段。
所以"变慢"不能直接归因于 n=4，尺度无关的那几个比值才是可比的，而它们说的是"没变"。

顺带确认夹爪量程 bug 仍在：左夹爪 state 最大 **1.2091**（训练界 1.0），§9.1 未修。

### 11.5 结论：这两个旋钮到此为止

- `chunk_interval_s`：客户端这个参数在本部署下无效，别再动它；要调节拍去服务端。
- `n_action_steps`：4 和 8 在尺度无关指标上没有差别，往下调（2、1）只会让接缝更密，
  同样的机制预期同样无效。
- 接缝要真解决，只剩 **RTC 引擎**（让新 chunk 与旧 chunk 在重叠区对齐，
  直接消掉首帧偏移）这一条配置侧的路；
  否则就得走 §10 第 3 条的结构性修改，让模型自己不产生这个偏移。
- **§10 的优先级不变，但第 4 条的第一小项作废**：现在第 1 条（夹爪量程）和
  第 3 条（读出头 / 二阶差分平滑项）是仅存的两条有效路径。

### 11.6 复现

```bash
cd /home/kewei/YING/paper/policy/scripts_vita_chunk_0813_n4
conda run -n lerobot python an4.py    # 解析 + 全套指标 + n=8/n=4 对照
conda run -n lerobot python norm4.py  # 运动量归一化（尺度无关比值）
conda run -n lerobot python dec4.py   # 接缝份额分解 / 夹爪量程 / 每帧进展
```
